...
// 2. Counters (Filtered by 'office')
$stmtCounters = $conn->prepare("SELECT c.id, c.site_id, c.name as counter_name, c.status, s.name as site_name FROM counters c JOIN sites s ON c.site_id = s.id WHERE s.type = 'office' ORDER BY c.id DESC");
...
// Sites (Filtered by 'office')
$stmtSites = $conn->prepare("SELECT * FROM sites WHERE type = 'office' ORDER BY id DESC");
$stmtSites->execute();
$sites = [];
...
// Active Sites for Assignments (Filtered by 'office')
$stmtAssignments = $conn->prepare("SELECT id, name as site_name, type FROM sites WHERE type = 'office' AND is_archived = 0 ORDER BY id DESC");
...
$pageTitle = 'Office Services - Admin Portal';
$activeMenu = 'office_services';
...
<div class="main-content">
    ...
    <div class="tab-bar">
        ...
        <button class="tab-btn <?php echo $activeTab === 'settings' ? 'active' : ''; ?>" onclick="openTab('settings')">Settings</button>
    </div>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <?php if (!$row['is_archived']): ?>
                                        <button onclick="openArchiveModal('/modules/services/archive.php', 'service_id', <?php echo $row['id']; ?>, 'office_services', '<?php echo htmlspecialchars(addslashes($row['name'])); ?>')" style="cursor: pointer; color: orange; background: none; border: none; text-decoration: underline; padding: 0;">Archive</button>
                                    <?php endif; ?>
                                </td>
                            </tr>
                        <?php endforeach; ?>
    ...
    <div id="counters" class="tab-content <?php echo $activeTab === 'counters' ? 'active' : ''; ?>">
        <div style="margin-bottom: 15px;">
            <button onclick="openModal('addCounterModal')" style="color: blue; border: none; background: none; cursor: pointer; text-decoration: underline; font-size: 16px; padding: 0;">+ Add Counter</button>
        </div>
        <div class="table-container">
            <table>
                <thead>
                    ...
                    <?php if (count($counters) > 0): ?>
                        <?php foreach ($counters as $row): ?>
                            <tr>
                                ...
                                <td>
                                    <button onclick="openEditCounterModal(<?php echo $row['id']; ?>, '<?php echo htmlspecialchars(addslashes($row['counter_name'])); ?>', <?php echo $row['site_id']; ?>)" style="margin-right: 5px; color: blue; cursor: pointer; background: none; border: none; text-decoration: underline; padding: 0;">Edit</button>
                                    <button onclick="openAssignModal(<?php echo $row['id']; ?>, <?php echo $row['site_id'] ?? 'null'; ?>, '<?php echo htmlspecialchars(addslashes($row['counter_name'])); ?>', 'office_services')" style="margin-right: 5px; color: purple; cursor: pointer; background: none; border: none; text-decoration: underline; padding: 0;">Assign Services</button>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
    ...
    <div id="settings" class="tab-content <?php echo $activeTab === 'settings' ? 'active' : ''; ?>">
        ...
        <div class="settings-section">
            <h3>Office Sites</h3>
            <div style="margin-bottom: 15px;">
                <button onclick="openModal('addSiteModal')" style="color: blue; border: none; background: none; cursor: pointer; text-decoration: underline; font-size: 16px; padding: 0;">+ Add Office Site</button>
            </div>
            <div class="table-container">
                <table>
                    <thead>
                            ...
                            <?php foreach ($sites as $row): ?>
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <?php if (!$row['is_archived']): ?>
                                            <button onclick="openArchiveModal('/modules/sites/archive.php', 'site_id', <?php echo $row['id']; ?>, 'office_services', '<?php echo htmlspecialchars(addslashes($row['name'])); ?>')" style="cursor: pointer; color: orange; background: none; border: none; text-decoration: underline; padding: 0;">Archive</button>
                                        <?php endif; ?>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
        ...
        <div class="settings-section">
            <h3>Service Assignments (Office)</h3>
            <div class="table-container">
                <table>
                    <thead>
                        ...
                        <?php if (count($activeSites) > 0): ?>
                            <?php foreach ($activeSites as $row): ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <button onclick="openAssignSiteModal(<?php echo $row['id']; ?>, '<?php echo htmlspecialchars(addslashes($row['site_name'])); ?>', 'office_services')" style="margin-right: 5px; color: purple; cursor: pointer; background: none; border: none; text-decoration: underline; padding: 0;">Manage Assignments</button>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
    ...
    <div id="addServiceModal" class="modal">
        <div class="modal-content">
            ...
            <form action="/modules/services/action_add.php" method="POST">
                <input type="hidden" name="return_to" value="office_services">
    ...
    <div id="addCounterModal" class="modal">
        <div class="modal-content">
            <span class="close" onclick="closeModal('addCounterModal')">&times;</span>
            <h2>Add New Counter</h2>
            <form action="/modules/counters/action_add.php" method="POST">
                <input type="hidden" name="return_to" value="office_services">
                ...
                <?php if (count($activeSites) > 0): ?>
                    <input type="hidden" name="site_id" value="<?php echo $activeSites[0]['id']; ?>">
                <?php else: ?>
                    <div class="form-group" style="margin-bottom: 15px;">
                        <p style="color:red;">No active Office Sites found. Please add a site in the Settings tab first.</p>
                    </div>
                <?php endif; ?>
    ...
    <div id="editCounterModal" class="modal">
        <div class="modal-content">
            <span class="close" onclick="closeModal('editCounterModal')">&times;</span>
            <h2>Edit Counter</h2>
            <form action="/modules/counters/action_edit.php" method="POST">
                <input type="hidden" name="return_to" value="office_services">
                <input type="hidden" name="counter_id" id="edit_counter_id">
                <input type="hidden" name="site_id" id="edit_counter_site_id">
                
                <div class="form-group" style="margin-bottom: 15px;">
                    <label style="display: block; margin-bottom: 5px;">Counter / Window Name:</label>
                    <input type="text" name="counter_name" id="edit_counter_name" required style="width: 100%; padding: 8px;">
                </div>
    ...
    <div id="addSiteModal" class="modal">
        <div class="modal-content">
            ...
            <form action="/modules/sites/action_add.php" method="POST">
                <input type="hidden" name="return_to" value="office_services">
                <input type="hidden" name="site_type" value="office">
...
<script>
    ...
    function openEditCounterModal(id, name, siteId) {
        document.getElementById('edit_counter_id').value = id;
        document.getElementById('edit_counter_name').value = name;
        document.getElementById('edit_counter_site_id').value = siteId;
                ...
                if (data.success) {
                    ...
                    if (data.allServices.length === 0) {
                        ...
                        data.allServices.forEach(srv => {
                            let checked = data.assignedServices.includes(srv.id) ? 'checked' : '';
                            html += `<div style="margin-bottom: 5px;">
                                <label>
                                    <input type="checkbox" name="services[]" value="${srv.id}" ${checked}> 
                                    ${srv.name}
                                </label>
                            </div>`;
                        });
